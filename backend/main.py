...
information and request precise pass predictions computed with an
adaptive stepping algorithm.  The adaptive algorithm dramatically
reduces the number of orbit calculations required while retaining
accuracy, as motivated by recent research on satellite visibility
prediction【439591833264522†L90-L100】.
"""
...
app = FastAPI(
    ...
    description=(
        ...
        "visibility windows for observers on Earth.  Pass predictions are "
        "computed using an adaptive algorithm for efficiency."
    ),
)
...
async def predict_passes(payload: PassPredictionRequest) -> Dict[str, object]:
    ...
    # Ensure the TLE catalog is available
    try:
        await asyncio.to_thread(service.get_tle_catalog)
    except Exception as e:
        raise HTTPException(status_code=502, detail=str(e))
